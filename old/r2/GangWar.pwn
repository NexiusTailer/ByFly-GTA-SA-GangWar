...
#define DIALOG_GANG_STOCK				(DIALOG_GANG_ONLINE + 1)
#define DIALOG_GANG_STOCK_AMOUNT		(DIALOG_GANG_STOCK + 1)
#define DIALOG_GANG_STOCK_GIVE			(DIALOG_GANG_STOCK_AMOUNT + 1)
#define DIALOG_GANG_STOCK_TAKE			(DIALOG_GANG_STOCK_GIVE + 1)
#define DIALOG_GANG_BATTLE				(DIALOG_GANG_STOCK_TAKE + 1)
#define DIALOG_GANG_BATTLE_ACCEPT		(DIALOG_GANG_BATTLE + 1)
#define DIALOG_ADMINS					(DIALOG_GANG_BATTLE_ACCEPT + 1)
		...
		case DIALOG_GANGS_MENU:
		{
			if(response)
			{
				switch(listitem)
				{
					...
					{
						new string[64];
						format(string, sizeof string, "{FFFF00}Общак банды \"%s\"", GName[PlayerInfo[playerid][pGangId]]);
						ShowPlayerDialog(playerid, DIALOG_GANG_STOCK, DIALOG_STYLE_LIST, string, "Посмотреть деньги\nПоложить деньги\nВзять деньги", "Выбрать", "Назад");
					}
					case 2:
					{
		...
		case DIALOG_GANG_STOCK:
		{
			if(response)
			{
				switch(listitem)
				{
					case 0:
					{
						new string[64], string2[32];
						format(string, sizeof string, "{FFFF00}Общак банды \"%s\"", GName[PlayerInfo[playerid][pGangId]]);
						format(string2, sizeof string2, "Денег в общаке: $%d", GMoney[PlayerInfo[playerid][pGangId]]);
						ShowPlayerDialog(playerid, DIALOG_GANG_STOCK_AMOUNT, DIALOG_STYLE_MSGBOX, string, string2, "Ок", "");
					}
					case 1:
					{
						if(PlayerInfo[playerid][pGang] == PlayerInfo[playerid][pGangId]) ShowPlayerDialog(playerid, DIALOG_GANG_STOCK_GIVE, DIALOG_STYLE_INPUT, "{FFFF00}Положить деньги", "Укажите сумму, которую вы хотите положить в общак:", "Ок", "Назад");
						else SendClientMessage(playerid, COLOR_RED_GANG, "Вы должны состоять в этой банде.");
					}
					case 2:
					{
						if(PlayerInfo[playerid][pGang] == PlayerInfo[playerid][pGangId] && PlayerInfo[playerid][pGangLvl] == 6) ShowPlayerDialog(playerid, DIALOG_GANG_STOCK_TAKE, DIALOG_STYLE_INPUT, "{FFFF00}Взять деньги", "Укажите сумму, которую вы хотите взять с общака:", "Ок", "Назад");
						else SendClientMessage(playerid, COLOR_RED_GANG, "Вы должны состоять в этой банде, и у вас должен быть уровень лидера.");
					}
				}
			}
			else
