...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	PlayerInfo[playerid][pFloodTime] = gettime() + 1;
	static cmd[32], tmp[32], string[145];
	string[0] = EOS;
	new idx;
	...
	if(!strcmp(cmd, "/toprace", true))
	{
		...
		if(iniFile >= INI_OK)
		{
			static string3[64], name[MAX_PLAYER_NAME];
			string[0] = EOS;
	...
	if(!strcmp(cmd, "/sethp", true))
	{
		...
		if(!tmp[0]) return SendClientMessage(playerid, COLOR_RED, "Используйте: /sethp [id] [кол-во]");
		new hp = strval(tmp);
		printf("Админ %s установил здоровье игроку %s на %d", PlayerInfo[playerid][pName], PlayerInfo[player][pName], hp);
